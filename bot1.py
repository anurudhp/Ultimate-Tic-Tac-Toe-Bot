...
class Manual_Player:
	...
	def gainEstimate(self, cd, cx, co):
		gain = 0
		if(cx == 3 and cd == 1 and co == 0):
			gain = 100
		if(co==3 and cd==1 and cx==0):
			gain = -100
		if(cx==2 and cd==2 and co==0):
			gain = 10
		if(co==2 and cd==2 and cx==0):
			gain = -10
		if(cx==1 and cd==3 and co==0):
			gain = 1
		if(co==1 and cd==3 and cx==0):
			gain = -1
	...
	def evaluation(self, board, old_move):
		...
		#if(goal_flag==1)
		#row checks
		#number of xs and os in active block
		gain = 0
		for i in range(0, 4):
			countx = 0
			counto = 0
			...
			#condition checking per row of active block
			if countx == 4:
				gain += 100000
				goal_flag = 0
				break
		...
		for j in range(0, 4):
			...
			#condition checking per column of active block
			if countx == 4 :
				gain += 100000
				goal_flag = 0
				break
		...
		#condition checking of diagonal 1 of active block
		if countx == 4 :
			gain += 100000
			goal_flag = 0
		...
		#condition checking of diagonal 2 of active block
		if countx == 4:
			gain += 100000
			goal_flag = 0
		elif counto == 4:
			...
			for checky in range(0, 13, 4):
				for i in range(0, 4):
					...
					if countx == 4:
						...
						gain += self.gainEstimate(countd, countx, counto)
	...
	def minimax_search(self, board, old_move, depth, alpha, beta, max_player, flag):
		...
		#print status
		if depth == 2 or status[0] != 'CONTINUE':
