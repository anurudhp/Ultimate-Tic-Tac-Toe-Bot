...
class Player52():
	...
	def move(self, board, old_move, flag):
		...
		#clock the move in 13 seconds to prevent time out
		signal.alarm(CLOCK)
		for cell in cells:
			if flag=='x':
				fl = 'o'
			...
			try:
				...
				if val > maxval:
					maxval = val
					bm = cell
				if bm == []:		#randomly get a move if no given specific move
					if val == maxval:
						if count == rd:
							bm = cell					#try to get player 1's move			
			except TimedOutExc:
				if bm == []:
					return cells[rd]
				else:
...
#checking for 2 continuous x/o in a column/row/diagonal of the block_status for min player
	...
	def find_valid_move_cells_optimised(self, board, old_move, flag, isMax):
		...
		#self.update_end_moves( board, allowed_cells, flag, isMax)
		best_allowed_moves = []
